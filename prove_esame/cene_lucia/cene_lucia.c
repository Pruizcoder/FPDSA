...
*/
    ...
    while (fgets(s,MAX_STRING_DIM, fp_presenze) != NULL)
    {
       ...
       printf("dentro while");
        ...
        printf("%d-%d-%d-%s", presenza.data.giorno,presenza.data.mese,presenza.data.anno, presenza.quando);
 ...
 struct proposta estrai_dati(char s[])
 {
       ...
       i= i+3; //salto i due punti e gli spazi
        //estrai il giorno
while ((s[i] < '0')|| (s[i] > '9'))
...
int cerca_data(struct proposta DateDisponibili[],struct proposta presenze, int nDateDisponibili)
{
    ...
    {
       if (DateDisponibili[i].data.giorno == presenze.data.giorno
        && DateDisponibili[i].data.mese == presenze.data.mese 
        && DateDisponibili[i].data.anno == presenze.data.anno
        && !(strcmp(DateDisponibili[i].quando, presenze.quando))==0)
       return i; 
